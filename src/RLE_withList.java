...
public class RLE_withList {
    ...
    public static void decompress(InputStream is, OutputStream os) throws Exception {
        ...
        while (true) {
            ...
            // --------------------------------------------------------- //
                ...
                // ----------------------------------------------------- //
                if (count == 2) {operationDecompress(last, current); continue;}
                else  LIST.add(last);
                // ----------------------------------------------------- //
                count = 1;
                last = current;
    ...
    // ------------------------------------------------------------------ //
    // ------------------------------------------------------------------ //
    private static List operationDecompress(int last,int current) {
        //if (count > 255) { maxCapacityByte(last,count);}

        if (current != 0)
            for (int i = 0; i < current; i++) { LIST.add(last); }
         else
             for (int i = 0; i < MAX_NUMBER_REPEAT; i++) { LIST.add(last); }
